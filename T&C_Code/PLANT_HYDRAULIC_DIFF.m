...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%REFERENCES
function [dX]=PLANT_HYDRAULIC_DIFF(t,X,Ccrown,Psi_s,T,hc,LAI,Axyl,PsiL50,PsiL00,Kleaf_max,Cl,Kx_max,PsiX50,Cx,gsr,dt,V_avail)
%%% INPUTS
dX=zeros(4,1);
Psi_x = X(1); Psi_l = X(2); % Vx=X(3);  Vl=X(4);
%%%%%%%%%%%%%%
%%%%%%%%%%%%%
...
if Axyl > 0
    %%%%% Fluxes
    Jsx = min(V_avail/dt,((2*Kx*gsr)/(2*Kx + gsr))*(Psi_s - Psi_x)); %% [mmolH20 /m^2 PFT s ]
    if LAI == 0 
        Jxl = 0;
        dX(1) = (Jsx-Jxl)/Cx; %  [MPa/s]
        dX(3) = (Jsx - Jxl);  
        dX(4) = (Jxl - T); %% [mmolH20/ m2 PFT s];
    else
        Jxl = ((2*Kx*Kleaf)/(2*Kx+Kleaf))*(Psi_x - Psi_l); %% [mmolH20/m^2 PFT  s]
        ...
        %Vx = Vxtm1 + dt*(Jsx - Jxl); %% 
        %Vl = Vltm1 + dt*(Jxl - T); %% 
        dX(3) = (Jsx - Jxl); % %  [mmolH20/m^2 PFT s]
        dX(4) = (Jxl - T); %% [mmolH20/ m2 PFT s];
    end
else   
    if LAI == 0 
        Jsl=0;
        dX(4) = (Jsl - T); %%% [mmolH20/ m2 PFT s];
    else
        %Jsl = ((2*Kleaf*gsr)/(2*Kleaf + gsr))*(Psi_s - Psi_ltm1); %% [mmolH20 /m^2 PFT s ]
        %Psi_l = Psi_ltm1 + dt*(Jsl - T)/Cl;
        %Vl = Vltm1 + dt*(Jsl - T)/rho2; %% [m3/ m2 PFT];
        Jsl = min(V_avail/dt,((2*Kleaf*gsr)/(2*Kleaf + gsr))*(Psi_s - Psi_l)); %% [mmolH20 /m^2 PFT s ]
        dX(2) = (Jsl - T)/Cl; %  [MPa/s]
        dX(4) = (Jsl - T); %% [mmolH20/ m2 PFT s];
        %%%
        %Jsx = Jsl; Jxl = Jsl;
