function[aR,Zs,...
    ...
    Deb_Par,Zs_deb,... 
    Sllit,Kct,ExEM,ParEx_H,Mpar_H,ParEx_L,Mpar_L]=PARAMETERS_SOIL_Gletsch(ANSWER,Psan,Pcla,Porg,dbThick, md_max,zatm_surface,Aice_meas_on,Ameas_t)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
ms=11; %%% Soil Layer
zatm =[15.5 zatm_surface zatm_surface zatm_surface] ; %% Reference Height
zatm = zatm(ANSWER);
fpr = 1;
%%%%
aR =100;
...
Krock = [NaN NaN NaN 0.15 ]; %% [mm/h] Conductivity of Fractured Rock
Kbot= Kbot(ANSWER);
Krock=Krock(ANSWER);
%%%%%%%%%% SOIL INPUT
Color_Class = 0;
...
switch ANSWER
    case 1
        %%%% LAND COVER PARTITION  forest - evergreen
    ...
    case 2
        ...
        II = [ 0 1 0 ]>0;
    case 3
        %%%% LAND COVER PARTITION  grass meadow
        Cwat = 0; Curb = 0.0 ; Crock = 0.0;
...
if Aice_meas_on==1
    Aice = Ameas_t; %Use measured albedo
    Deb_Par.alb = Ameas_t; %Use measured albedo
else
    Aice = 0.28; %% [-] Ice albedo Use standard albedo
end
...
Do_L=[1000 1200 1000]; %%[Pa]
a1_L=[6 9 6 ];
go_L=[0.01 0.01 0.01 ];% % [mol / s m^2] minimum Stomatal Conductance
CT_L=[3 3 3];  %%--> 'CT' == 3  'CT' ==  4  %% Photosyntesis Typology for Plants
DSE_L =[0.656 0.66 0.656 ];  %% [kJ/mol] Activation Energy - Plant Dependent
Ha_L =[55 70 55 ]; %% [kJ / mol K]  entropy factor - Plant Dependent
gmes_L=[Inf Inf Inf ]; %% [mol CO2 / s m^2 ];  mesophyll conductance
rjv_L=[2.4 2.5 2.4 ]; %%% Scaling Jmax - Vmax  [umol electrons / umolCO2 ]
%%%
FI_L=FI_L(II); Do_L=Do_L(II); a1_L=a1_L(II); go_L=go_L(II);
CT_L=CT_L(II); DSE_L=DSE_L(II); Ha_L=Ha_L(II); gmes_L=gmes_L(II);
rjv_L=rjv_L(II);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Psi_sto_50_H = [-2.5 -2.8 -2.0] ;%% [MPa]  Water Potential at 50% loss conductivity
Psi_sto_00_H = [-0.5 -0.8 -0.5]; %% [MPa]  Water Potential at 2% loss conductivity
%%% Leaf
PsiL00_H = [ -1.0 -1.0 -2.7] ;%%[MPa]  Water Potential at 50% loss conductivity
PsiL50_H =  [-3.2 -3.5 -5.2]; %% [MPa]  Water Potential at 2% loss conductivity
...
%%------------------------
%
Psi_sto_00_L = [0 -0.8 -0.5];%  %% [MPa]  Water Potential at PLCs loss conductivity
Psi_sto_50_L = [ 0 -2.8 -3.0];%  %% [MPa]  Water Potential at 50% loss conductivity
%%% Leaf
PsiL00_L =  [ 0 -1.0 -0.9]; %% [MPa]  Water Potential at PLCs% loss conductivity
PsiL50_L = [0 -3.5 -4.0] ;%%[MPa]  Water Potential at 50% loss conductivity
Kleaf_max_L = [0 5 5] ; %%  %%%  [mmolH20 m^2 leaf s /MPa]
Cl_L  = [ 0 1200 1200];  %%%  [500 - 3000]%  [mmolH20 / m^2 leaf MPa]
%%% Xylem
Axyl_L = [0 2 0.0] ; %% [cm^2 stem /m^2 PFT]
Kx_max_L = [ 0 80000 80000];  %%5550-555550 [mmolH20 /m s MPa]  Xylem Conductivity specific for water;
PsiX50_L = [ 0 -4.5 -4.5]; %%[MPa]  Water Potential at 50% loss conductivity
Cx_L= [0 150 150]; %%% [kg / m^3 sapwood MPa]
%%%
...
%%% LOW VEGETATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Sl_L = [ 0.035 0.024 0.035 ]; % 0.05 -0.005 [m^2 gC] specific leaf area of  biomass [m^2 /gC]
Nl_L= [23 28 23  ]; %[gC/gN ] Leaf Carbon-Nitrogen ratio
r_L = [0.060 0.045 0.060 ];  %% [0.066 -0.011]respiration rate at 10° [gC/gN d ]
gR_L= [0.25 0.25 0.25  ]; % [0.22 - 0.28] growth respiration  [] -- [Rg/(GPP-Rm)]
aSE_L= [2 1 2 ]; %%% Plant Type -- 1 Seasonal Plant --  0 Evergreen  -- 2 Grass species -- 3 Crops
dd_max_L= [ 1/45 1/365 1/45];%%%0.005  [1/d]  0.0250 -- 0.005-0.025 death maximum for drought
dc_C_L =  [7/365 32/365 7/365 ]; %% [1/ d°C] -- [Factor of increasing mortality]
Tcold_L = [-2.0 0.0 -2.0 ]; %% [°C] Cold Leaf SLed
drn_L=  [1/450 1/900 1/450 ]; %% turnover root  [1/d]
dsn_L= [ 1/365 1/500 1/365 ]; % normal transfer rate sapwood [1/d]
age_cr_L= [180 120 180 ]; %% [day] Critical Leaf Age
Bfac_lo_L= [0.99 0.99 0.99 ]; %% Leaf Onset Water Stress
Bfac_ls_L= [NaN NaN NaN ]; %% Leaf Shed Water Stress [0-1]
Tlo_L = [0.0 4.0 0.0 ]; %% Mean Temperature for Leaf onset
Tls_L = [NaN NaN NaN ]; %% Mean Temperature for Leaf Shed
PAR_th_L= [NaN NaN NaN];
dmg_L= [20 35 20 ]; %%% Tree 30 Grasses Day of Max Growth
LAI_min_L = [0.1 0.01 0.1 ];
Trr_L = [2.0 2.0 2.0 ]; %% Translocation rate [gC /m^2 d]
mjDay_L = [250 250 250 ]; %% Maximum Julian day for leaf onset
LDay_min_L =[10.7 12.8 10.7 ]; %% Minimum Day duration for leaf onset
LtR_L = [0.35 0.8 0.35  ]; %%% Leaf to Root ratio maximum
Mf_L= [1/50 1/50 1/50 ]; %% fruit maturation turnover [1/d]
Wm_L= [0 0 0 ] ; % wood turnover coefficient [1/d]
eps_ac_L = [0.2 1 0.2  ]; %% Allocation to reserve parameter [0-1]
LDay_cr_L = [10.7 10.8 10.7 ]; %%%  Threshold for senescence day light [h]
Klf_L =[1/50 1/28 1/50 ]; %% Dead Leaves fall turnover [1/d]
fab_L = [ 0 0.5 0 ]; %% fraction above-ground sapwood and reserve
fbe_L = [ 1 0.5 1 ]; %% fraction below-ground sapwood and reserve
ff_r_L= [0.1 0.1 0.1]; %%
%%%
