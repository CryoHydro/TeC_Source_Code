...
AgeL_L=zeros(NNd,cc);e_rel_L=ones(NNd,cc);SAI_L=zeros(NNd,cc); hc_L=zeros(NNd,cc); e_relN_L=ones(NNd,cc);
%%%
...
AgeL_H=zeros(NNd,cc);e_rel_H=ones(NNd,cc);SAI_H=zeros(NNd,cc); hc_H=zeros(NNd,cc); e_relN_H=ones(NNd,cc);
%%%
...
for i=2:NN
    ...
    if (Datam(i,4)==1)
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%% Biogeochemistry
        [Se_bio,Se_fc,Psi_bio,Tdp_bio,VSUM,VTSUM]=Biogeo_environment(Tdp(pdind,:),O(pdind,:),V(pdind,:),Osat,Ohy,nVG,alpVG,Ks_Zs,L,Pe,O33,Ofc,SPAR,Bio_Zs);% sum(V(i,:))
        ...
        for cc=1:length(Ccrown)
            ...
            if ZR95_H(cc) > 0
                [LAI_H(j,cc),B_H(j,cc,:),NPP_H(j,cc),ANPP_H(j,cc),Rg_H(j,cc),RA_H(j,cc),Rms_H(j,cc),Rmr_H(j,cc),Rmc_H(j,cc),PHE_S_H(j,cc),...
                    ...
                    Ta(pdind),Tdp_H(pdind,cc),PARB(pdind)+PARD(pdind),Psi_x_H(pdind,cc),Psi_l_H(pdind,cc),An_H(pdind,cc),Rdark_H(pdind,cc),PsiL50_H(cc),PsiL00_H(cc),PsiX50_H(cc),NPP_H(j-1,cc),jDay(j),Datam(i,:),...
                    NPPI_H(j-1,cc),TdpI_H(j-1,cc),Bfac_weekH(j-1,cc),NupI_H(j-1,cc,:),NavlI(j,:),PARI_H(j-1,cc,:),NBLI_H(j-1,cc),NBLeaf_H(j-1,cc),...
                    Sl_H(cc),mSl_H(cc),Stoich_H(cc),r_H(cc),gR_H(cc),LtR_H(cc),eps_ac_H(cc),aSE_H(cc),Trr_H(cc),...
                    dd_max_H(cc),dc_C_H(cc),Tcold_H(cc),drn_H(cc),dsn_H(cc),age_cr_H(cc),Bfac_lo_H(cc),Bfac_ls_H(cc),PAR_th_H(cc),...
                    Tlo_H(cc),Tls_H(cc),mjDay_H(cc),LDay_min_H(cc),dmg_H(cc),Mf_H(cc),Wm_H(cc),LAI_min_H(cc),L_day(j),LDay_cr_H(cc),Lmax_day,...
                    Nreserve_H(j-1,cc),Preserve_H(j-1,cc),Kreserve_H(j-1,cc),Nuptake_H(j,cc),Puptake_H(j,cc),Kuptake_H(j,cc),FNC_H(j-1,cc),Se_bio,Tdp_bio,PsiG50_H(cc),PsiG99_H(cc),gcoef_H(cc),...
                    fab_H(cc),fbe_H(cc),Klf_H(cc),ff_r_H(cc),ParEx_H(cc),ExEM,Mpar_H(cc),TBio_H(cc),OPT_EnvLimitGrowth,OPT_VD,OPT_SoilBiogeochemistry);
                %%%%%%%%%%%%%%%%%%
                [TexC_H(j,cc),TexN_H(j,cc),TexP_H(j,cc),TexK_H(j,cc),TNIT_H(j,cc),TPHO_H(j,cc),TPOT_H(j,cc),NuLit_H(j,cc,:),Nreserve_H(j,cc),Preserve_H(j,cc),Kreserve_H(j,cc),...
                    SupN_H(j,cc),SupP_H(j,cc),SupK_H(j,cc),ISOIL_H(j,cc,:)]= Plant_Exports(B_H(j,cc,:),NuLit_H(j-1,cc,:),...
                ...
                if aSE_H(cc) == 2
                    ...
                    if OPT_VCA == 1
                        %%%%%%
                        [Ccrown_t(j,cc),hc_H(j,cc),SAI_H(j,cc),Tden_H(j,cc),AgePl_H(j,cc)]=Vegetation_Structural_Attributes(dtd,...
                            Ccrown_t(j-1,cc),B_H(j,cc,:),fab_H(cc),Tden_H(j-1,cc),AgePl_H(j-1,cc));
                        %%%%
                        Ccrown(cc) = Ccrown_t(j,cc); 
                        %%%%
            ...
            if ZR95_L(cc) > 0
                [LAI_L(j,cc),B_L(j,cc,:),NPP_L(j,cc),ANPP_L(j,cc),Rg_L(j,cc),RA_L(j,cc),Rms_L(j,cc),Rmr_L(j,cc),Rmc_L(j,cc),PHE_S_L(j,cc),...
                    ...
                    Ta(pdind),Tdp_L(pdind,cc),PARB(pdind)+PARD(pdind),Psi_x_L(pdind,cc),Psi_l_L(pdind,cc),An_L(pdind,cc),Rdark_L(pdind,cc),PsiL50_L(cc),PsiL00_L(cc),PsiX50_L(cc),NPP_L(j-1,cc),jDay(j),Datam(i,:),...
                    NPPI_L(j-1,cc),TdpI_L(j-1,cc),Bfac_weekL(j-1,cc),NupI_L(j-1,cc,:),NavlI(j,:),PARI_L(j-1,cc,:),NBLI_L(j-1,cc),NBLeaf_L(j-1,cc),...
                    Sl_L(cc),mSl_L(cc),Stoich_L(cc),r_L(cc),gR_L(cc),LtR_L(cc),eps_ac_L(cc),aSE_L(cc),Trr_L(cc),...
                    dd_max_L(cc),dc_C_L(cc),Tcold_L(cc),drn_L(cc),dsn_L(cc),age_cr_L(cc),Bfac_lo_L(cc),Bfac_ls_L(cc),PAR_th_L(cc),...
                    Tlo_L(cc),Tls_L(cc),mjDay_L(cc),LDay_min_L(cc),dmg_L(cc),Mf_L(cc),Wm_L(cc),LAI_min_L(cc),L_day(j),LDay_cr_L(cc),Lmax_day,...
                    Nreserve_L(j-1,cc),Preserve_L(j-1,cc),Kreserve_L(j-1,cc),Nuptake_L(j,cc),Puptake_L(j,cc),Kuptake_L(j,cc),FNC_L(j-1,cc),Se_bio,Tdp_bio,PsiG50_L(cc),PsiG99_L(cc),gcoef_L(cc),...
                    fab_L(cc),fbe_L(cc),Klf_L(cc),ff_r_L(cc),ParEx_L(cc),ExEM,Mpar_L(cc),TBio_L(cc),OPT_EnvLimitGrowth,OPT_VD,OPT_SoilBiogeochemistry);
                %%%%%%%%%%%%%%%%%%%
                [TexC_L(j,cc),TexN_L(j,cc),TexP_L(j,cc),TexK_L(j,cc),TNIT_L(j,cc),TPHO_L(j,cc),TPOT_L(j,cc),NuLit_L(j,cc,:),Nreserve_L(j,cc),Preserve_L(j,cc),Kreserve_L(j,cc),...
                    SupN_L(j,cc),SupP_L(j,cc),SupK_L(j,cc),ISOIL_L(j,cc,:)]= Plant_Exports(B_L(j,cc,:),NuLit_L(j-1,cc,:),...
                ...
                if aSE_L(cc) == 2
                    ...
                    if OPT_VCA == 1
                        %%%%%%
                        [Ccrown_t(j,cc),hc_L(j,cc),SAI_L(j,cc),Tden_L(j,cc),AgePl_L(j,cc)]=Vegetation_Structural_Attributes(dtd,...
                            Ccrown_t(j-1,cc),B_L(j,cc,:),fab_L(cc),Tden_L(j-1,cc),AgePl_L(j-1,cc));
                        %%%%
                        Ccrown(cc) = Ccrown_t(j,cc); 
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [V(i,:),O(i,:),ZWT(i),OF(i),OS(i),OH(i,:),OL(i,:),Psi_s_H(i,:),Psi_s_L(i,:),Rd(i),Qi_out(i,:),WTR(i,:),...
        ...
        G(i),Gfin(i),Tdp(i,:),Sdp(i,:),Tdamp(i),Tdp_H(i,:),Tdp_L(i,:),SWE(i),SND(i),ros(i),In_SWE(i),SP_wc(i),WR_SP(i),U_SWE(i),NIn_SWE(i),dQ(i),Qfm(i),t_sls(i),DQ(i),DT(i),...
        WAT(i),ICE(i),ICE_D(i),IP_wc(i),WR_IP(i),NIce(i),Cicew(i),Csnow(i),FROCK(i),...
        ...
        snow_alb,tau_sno(i),e_sno(i),Ws_under(i),dQVEG(i),HV(i),QEV(i),TsV(i),EK(i),POT(i,:)]=HYDROLOGIC_UNIT(V(i-1,:),aR,Zs,...
        EvL_Zs,Inf_Zs,Zinf,RfH_Zs,RfL_Zs,dz,Ks_Zs,Dz,ms,Kbot,Pr(i),Ta(i),Ds(i),Ws(i),zatm,Ts(i-1),dt,dth,ea(i),N(i),Pre(i),Tstm0,...
        ...
        gcI,KcI,TminS,TmaxS,...
        KnitH,KnitL,mSl_H,Sl_H,mSl_L,Sl_L,Ca(i),Oa,Ci_sunH(i-1,:),Ci_shdH(i-1,:),CT_H,Ci_sunL(i-1,:),Ci_shdL(i-1,:),CT_L,Vmax_H,Vmax_L,FI_H,FI_L,a1_H,go_H,a1_L,go_L,...
        DSE_H,Ha_H,Do_H,e_rel_H(j,:),e_relN_H(j,:),DSE_L,Ha_L,Do_L,e_rel_L(j,:),e_relN_L(j,:),gmes_H,rjv_H,gmes_L,rjv_L,...
        e_sno(i-1),In_H(i-1,:),In_L(i-1,:),In_Litter(i-1),In_urb(i-1),In_rock(i-1),SWE(i-1),In_SWE(i-1),....
        Sdp(i-1,:),Tdp(i-1,:),Tdamp(i-1),rsd,lan_dry,lan_s,cv_s,...
        WAT(i-1),ICE(i-1),IP_wc(i-1),ICE_D(i-1),Ice_wc_sp,ros_Ice_thr,Aice,Cicew(i-1),...
        ...
        Tdew(i),t_sls(i-1),ros(i-1),SP_wc(i-1),fpr,WatFreez_Th,dz_ice,...
        Pr_sno(pdind),Th_Pr_sno,ros_max1,ros_max2,...
        In_max_urb,In_max_rock,K_usle,tau_sno(i-1),Slo_top,Slo_pot,Asur,Ared,aTop,EK(i-1),q_runon(i),Qi_in(i,:),...
        pow_dis,a_dis,...
        SPAR,SN,OPT_VegSnow,OPT_SoilTemp,OPT_PlantHydr,Opt_CR,Opt_ST,OPT_SM,OPT_STh,OPT_PH);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%
    Tstm0 =2*Ts(i)-Ts(i-1);
    %%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% v-coordinate
    CK1(i) = f(i)*dth*Asur*Ared + sum(V(i-1,:) - V(i,:))*Asur*Ared - EG(i)*dth - Lk(i)*dth ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% MASS BALANCE CHECK
dV= (V(1,:)-V(end,:))*Asur*Ared;
Ck = sum((Pr_liq(2:end)+Pr_sno(2:end))*dth) + sum(dV)...
