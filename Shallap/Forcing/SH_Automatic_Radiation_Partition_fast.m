...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function[SAD1,SAD2,SAB1,SAB2,PARB,PARD, A,Rsws,Nsim,Prw,Tatm1,Tatm2] = SH_Automatic_Radiation_Partition_fast(Date,Lat,Lon,Zbas,...
    DeltaGMT,Pr,Tdew,Rsw,t_bef,t_aft)

%EDITED by CFyffe to allow scalar inputs
...
for i=1:NT
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [SB(i),SD(i),SAB1(i),SAB2(i),SAD1(i),SAD2(i),PARB(i),PARD(i),Prw(i),Tatm1(i),Tatm2(i)]=ComputeRadiationForcingsN(Datam(i,:),DeltaGMT,Lon,Lat,...
        LWP0,N(i),Zbas,Tdew(i),beta_A,alpha_A,omega_A1,omega_A2,uo,un,rho_g,t_bef,t_aft);
end
Rsws=SB+SD;
[Nsim, A, Rsws]=Estimate_CloudCover(Rsw,Rsws,Pr,D); 
D=Date; N=Nsim;
N(isnan(Rsw))=0.15;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function[Nsim, A, Rsws]=Estimate_CloudCover(Rsw,Rsws,Pr,D)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
x=1:length(Nsim);
xnt= x(not(isnan(Nsim))); %Index of where there is data
xn=x(isnan(Nsim)); %Index of nans
Nn=interp1(xnt,Nsim(not(isnan(Nsim))),xn,'linear'); %interpolates where there are NaNs
Nsim(xn)=Nn; %Adds interpolated back in
Nsim(Pr>0.1)=1;
end 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Subfunction  Compute radiation forcings %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function[EB,ED,EB1,EB2,ED1,ED2,PARB,PARD,Prw,Tatm1,Tatm2]=ComputeRadiationForcingsN(Datam,DeltaGMT,Lon,Lat,...
...
if isequal(Ntmp,0) % Completely clear sky
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [EB1,EB2,ED1,ED2,Edp1,Edp2,rho_s1,rho_s2,Mb,Mg,Prw,Tatm1,Tatm2] = SetClearSkyRadiation(h_S,Zbas,Tdew,So1,So2,beta_A,alpha_A,omega_A1,omega_A2,uo,un,rho_g);
    %%%%%%%%%%%%%%%%%%%%%%
...
%     Subfunction SetClearSkyRadiation      %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Eb1,Eb2,Ed1,Ed2,Edp1,Edp2,rho_s1,rho_s2,Mb,Mg,Prw,Tatm1,Tatm2] = SetClearSkyRadiation(h_S,Zbas,Tdew,So1,So2,beta_A,alpha_A,omega_A1,omega_A2,uo,un,rho_g)
%%%% INPUT
%%h_S,[rad]  solar altitude
%Zbas [m a.s.l.]  watershed elevation
%Tdew [�C] dew point temperature
...
Prw = w;            % precipitable water [cm]
Tatm1 = prod(T1);   % atmospheric transmissivity VIS
Tatm2 = prod(T2);   % atmospheric transmissivity NIR
